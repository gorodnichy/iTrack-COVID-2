...
sliderInput("showN",
            ...
            # "Find top:",
            min = 1, max = 50, #dtGeo$country%>% unique %>% length/2,
            pre  = "top ",
            post  = " regions",
            # post  = " results",
            value = 20)
...
r.dt  <- reactive({
  ...
  # dtGeo000 <- dtGeo[ country %in% input$country &  state %in% input$state ] # input$state <- 'Quebec'
  ...
  # dtAll [date==input$dateToday] [state == input$state ]
  # dt <- dtAll [date==input$dateToday] [dtGeo000, on =c (colsGeo)];dt
  # No  dt <- dtGeo000 [ dtAll [date==input$dateToday], on =c (colsGeo)];dt
...
radioButtons("fRadio", "Compute: ", # LPlot: Metric Functional:", # "f", "Functionals:", Show Compute
             c(
               "Raw data (records/day)" = "",
               "Total" = "Total",
               "Per day Speed" = "Speed",
               "Per day Acceleration" = "Accel.",
               "Per day Growth" = "Growth.",
               "Per day Growth Change" = "Growth.Accel"
             ), inline=F,
             selected = "Speed"
)
...
checkboxInput("scale", "Fix scale constant", F)
  ...
  if (  dt00$region %>% unique %>% length > 0) {
    g <<- ggplot( dt00  ) + 
      ...
      labs(
        ...
        title= paste0("Number of infected cases ", input$fRadio, " over time"),
