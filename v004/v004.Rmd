...
r.dt00  <- reactive({
  ...
  dt00 <<- dtGeo[, .(state, country , population)][
    dtJHU[ country == input$country ]  , on=c("country", "state")]
  
  
  dt00 [, active := confirmed - ifelse(is.na(recovered), 0, recovered) - ifelse(is.na(death), 0, death)]
  
  dt00 [, activeTotal := confirmedTotal - ifelse(is.na(recoveredTotal), 0, recoveredTotal) - ifelse(is.na(deathTotal), 0, deathTotal)]
  
  
  cols3 <- c("confirmed","death",  "recovered")
  colsPer100K <- paste0(cols3, "Per100K")
  dt00[ , (colsPer100K):= lapply(.SD, function(x) {as.integer(x/population*1000000)}), by=c("country", "state"), .SDcols=cols3][.N]

  
  cols3total <- paste0(cols3, "Total")
  colsTotalPer100K <- paste0(cols3total, "Per100K")
  dt00[ , (colsTotalPer100K):= lapply(.SD, function(x) {as.integer(x/population*1000000)}), by=c("country", "state"), .SDcols=cols3total][.N]
...
renderPlotly({
    ...
    # 
    # ggplot(dt00) + 
    #   geom_col(aes(state, active), col="blue") + 
    #   geom_col(aes(state, active), col="blue") + )
  
  plotly::plot_ly(data = dt00[date==dateMax], #r.dtToday(), #r.dtCanada(), # df[1:30,],
                  x = ~ state, #date, # country,
                  y = ~ active, #confirmed, # (confirmed-recovered-death),
                  # text =  ~ confirmed,
                  # textposition = 'auto',
                  type = "bar",
                  name = "Active",
...
renderPlot( {
  ...
  g <<- ggplot(dt00) +  
    ...
    # geom_line(aes_string("date", cols3[3]), alpha=0.5, col="red", size=1) +
    ...
    #  title=paste0("", input$country), # ": Historical vs. Predicted"),
     # subtitle= "Confirmed, recovered, death dynamics",
      caption=paste0("Generated on ",  format(Sys.time(), "%d %B, %Y") ," by iTrack Covid (https://itrack.shinyapps.io/covid)"),
      #subtitle=paste0("Dates: ", dtJHU$date %>% min, " - ", dtJHU$date %>% max),
