---
title: "[iTrack COVID](https://itrack.shinyapps.io/covid/)"
...
r.dt  <- reactive({
  ...
  # No, no, dt <- dtGeo000 [ dtAll [date==input$dateToday], on =c (colsGeo)];dt
  
  dt <- dtAll [dtGeo000, on =c (colsGeo)];dt
  
  dt <- addDerivatives(dt, colsCases, colsGeo, input$convolution-1)
  dt[date==input$dateToday] 
  
  
  # Today -----[ [date == input$dateToday] 
  
  # dtN <- dt[date==input$dateToday] [order(-get(input$sortby))] [1:input$showN]; 
  
  # if (input$showN > 0 ) {
  
  if (T) {    
    dtN <- dt[date==input$dateToday] [order( ifelse(input$ascending, 1, -1)*get(input$sortby))] [1:input$showN]; 
    ...
    topNregions <- dtN$region;topNregions
    dt <- dt[region %in% topNregions]
