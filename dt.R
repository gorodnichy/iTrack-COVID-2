...
if (T) {
  ...
  sum.na0 <- function(x) { sum(x, na.rm = T) }
  median.na0 <- function(x) { median(x, na.rm = T) }
  mean.na0 <- function(x) { mean(x, na.rm = T) }
  ...
  dt.replace <- function (dt, FUNCTION = function (x) dplyr::if_else(is.na(x), 0L, x)) {
  ...
  dt.replaceNA2 <- function (dt, value) {
  ...
  dt.replaceBA <- function(dt, col, b, a) {
    dt[get(col)==a, (col):=b]
    return(dt)
  }
